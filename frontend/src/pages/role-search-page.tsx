...
import { asCurrency, compactNumber, pluralize, toDisplayLabel } from "../lib/formatters";
      ...
      <section className="panel p-4 sm:p-5 md:p-6">
        <div className="flex flex-col gap-5 xl:flex-row xl:items-end xl:justify-between">
          <div className="max-w-2xl">
            ...
            <p className="mt-2 text-sm leading-6 text-slate-600 dark:text-slate-300">Search one role to view demand, salary, skills, locations, and trend.</p>
          </div>

          <form onSubmit={onSearch} className="grid w-full gap-3 xl:max-w-xl xl:grid-cols-[1fr_auto]">
            <input
              className="input-base"
              placeholder="Search role title (example: Data Engineer)"
              value={query}
        ...
        <div className="space-y-4">
          <section className="grid gap-3 sm:gap-4">
            <div className="panel p-4 sm:p-5 md:p-6">
              ...
              <h3 className="mt-2 break-words text-xl font-semibold text-slate-900 dark:text-white sm:text-2xl">{toDisplayLabel(submittedQuery)}</h3>
              <p className="mt-2 text-sm text-slate-600 dark:text-slate-300">Simple view of the current market for this role.</p>
              ...
              <div className="mt-5 grid gap-3 sm:grid-cols-2 xl:grid-cols-4">
                ...
                <div className="metric-surface p-4">
                  ...
                  <p className="mt-1 text-sm text-slate-500 dark:text-slate-400">based on disclosed salary</p>
                </div>
                <div className="metric-surface p-4">
                  ...
                  <p className="mt-1 text-sm text-slate-500 dark:text-slate-400">demand strength index</p>
                </div>
                <div className="metric-surface p-4">
                  <p className="text-xs uppercase tracking-[0.18em] text-slate-500 dark:text-slate-400">Lead skill</p>
                  <p className="mt-2 text-xl font-semibold text-slate-900 dark:text-white">
                    {leadSkill ? toDisplayLabel(leadSkill.skill) : "N/A"}
                  </p>
                  <p className="mt-1 text-sm text-slate-500 dark:text-slate-400">
                    {leadSkill ? `seen in ${pluralize(leadSkill.count, "matching job")}` : "no strong signal yet"}
                  </p>
                </div>
              </div>
            </div>
              ...
              <div className="mt-4 grid gap-2 sm:grid-cols-2">
                {result.top_skills.length ? (
                  result.top_skills.map((item) => (
                    ...
                    >
                      <p className="font-medium">{toDisplayLabel(item.skill)}</p>
              ...
              <ul className="mt-4 space-y-2 text-sm text-slate-700 dark:text-slate-300">
                {result.top_locations.length ? (
                  result.top_locations.map((item) => (
                    ...
                    >
                      <div>
                        <p className="font-medium">{toDisplayLabel(item.skill) || "India"}</p>
          ...
          {!result.total_jobs ? (
            <div className="panel p-5 text-sm text-slate-600 dark:text-slate-300">
              No listings matched <span className="font-semibold">{toDisplayLabel(submittedQuery)}</span>. Try a broader title like <span className="font-semibold">Backend Developer</span>.
            </div>
          ) : null}
        </div>
      ) : null}
